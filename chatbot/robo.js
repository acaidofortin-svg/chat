...
// =====================================
client.on("qr", (qr) => {
  console.log("�x� Escaneie o QR Code:");
...
// =====================================
client.on("ready", () => {
  console.log("�S& BOT ONLINE COM SUCESSO");
});
...
client.on("auth_failure", (msg) => {
  ...
  qrDataUrl = null;
  qrPngBuffer = null;
  console.log("�R Falha de autentica��o:", msg);
  ...
  if (requestPath === "/bairros" && req.method === "POST") {
    ...
    req.on("end", () => {
      try {
        ...
        atualizarBairros(lista);
        salvarBairros(lista);
        console.log(`�S& Bairros sincronizados: ${lista.length}`);
...
const iniciarServidor = (portaInicial) => {
  ...
  servidor.on("error", (erro) => {
    ...
    console.log("�R Erro no servidor HTTP:", erro);
  });
...
Oi! Sou o assistente comercial do Açai do Fortin.
...
const mensagemAtendente = `�S& Certo! Vou pausar o rob� por 10 minutos para voc� conversar com o atendente.
...
const mensagemAgradecimento = `�x�` Que bom falar com voc�! Muito obrigado pelo carinho.
...
const mensagemConfirmacao = `Perfeito! �x�
...
const mensagemDespedida = `�x�` Combinado! Estaremos por aqui.
...
const buildSystemPrompt = () => {
  const base = [
    "Voce e o assistente comercial do Açai do Fortin.",
  ...
  try {
    ...
    if (session.etapa === "menu") {
      ...
      if (texto === "1") {
        ...
        await client.sendMessage(
          msg.from,
          "�xaa Me diga seu *bairro* para consultar a taxa e agilizar seu pedido."
        );
...
�x0 ${linkPrincipal}`
        ...
        await client.sendMessage(
          msg.from,
`�x�" Ainda n�o atendemos esse bairro.
