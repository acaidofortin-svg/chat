...
let ultimoReadyEm = 0;
let ultimoRestartAutomaticoEm = 0;
...
// =====================================
client.on("qr", (qr) => {
  console.log("�x�S� Escaneie o QR Code:");
...
client.on("ready", () => {
  ...
  ultimoReadyEm = Date.now();
});

client.on("authenticated", () => {
  console.log("�x� WhatsApp autenticado com sucesso");
});
...
client.on("auth_failure", (msg) => {
  ...
  qrDataUrl = null;
  qrPngBuffer = null;
  console.log("�� Falha de autentica��o:", msg);
...
client.on("disconnected", (reason) => {
  console.log("�a� WhatsApp desconectado:", reason);
  if (AUTO_REINICIAR_EM_FALHA) {
...
const iniciarCliente = async () => {
  ...
  try {
    ...
    if (erroIgnoravelPuppeteer(erro)) {
      console.log("Aviso: erro temporario do Puppeteer ignorado no inicio.");
      return;
    }
    if (AUTO_REINICIAR_EM_FALHA) {
      const precisaLimpar =
...
const erroIgnoravelPuppeteer = (erro) => {
...
const podeReiniciarAutomaticamente = () =>
  Date.now() - ultimoRestartAutomaticoEm > 5 * 60 * 1000;

const talvezRecuperarCliente = (erro) => {
  if (reinicioEmAndamento) return;
  if (!podeReiniciarAutomaticamente()) return;
  ultimoRestartAutomaticoEm = Date.now();
...
process.on("unhandledRejection", (reason) => {
  ...
  if (erroIgnoravelPuppeteer(reason)) {
    if (inicializandoCliente || !ultimoReadyEm) {
      console.log("Aviso: erro temporario do Puppeteer ignorado no inicio.");
      return;
    }
    console.log("Aviso: erro temporario do Puppeteer apos o ready. Tentando recuperar.");
    talvezRecuperarCliente(reason);
    return;
  }
  if (
...
const iniciarServidor = (portaInicial) => {
  ...
  servidor.on("error", (erro) => {
    if (erro.code === "EADDRINUSE") {
      ...
      portaAtual += 1;
      console.log(`�a� Porta ${antiga} em uso. Tentando porta ${portaAtual}...`);
...
const SESSAO_INATIVA_MS = 6 * 60 * 60 * 1000;
const ANTISPAM_INATIVO_MS = 10 * 60 * 1000;
const LIMPEZA_ESTADO_MS = 15 * 60 * 1000;

const registrarSessao = (id) => {
  const sessionAtual = sessions.get(id) || { etapa: "menu" };
  sessionAtual.updatedAt = Date.now();
  sessions.set(id, sessionAtual);
  return sessionAtual;
};

const limparEstadoMemoria = () => {
  const agora = Date.now();
  let sessoesRemovidas = 0;
  let antiSpamRemovidos = 0;

  for (const [id, session] of sessions.entries()) {
    if (!session?.updatedAt || agora - session.updatedAt > SESSAO_INATIVA_MS) {
      sessions.delete(id);
      sessoesRemovidas += 1;
    }
  }

  for (const [id, ultimo] of antiSpam.entries()) {
    if (!ultimo || agora - ultimo > ANTISPAM_INATIVO_MS) {
      antiSpam.delete(id);
      antiSpamRemovidos += 1;
    }
  }

  if (sessoesRemovidas || antiSpamRemovidos) {
    console.log(
      `Limpeza de memoria: ${sessoesRemovidas} sessoes e ${antiSpamRemovidos} entradas de anti-spam removidas.`
    );
  }
};

setInterval(limparEstadoMemoria, LIMPEZA_ESTADO_MS).unref?.();

// =====================================
// LINK DO CARDÁPIO
// =====================================
const linkPrincipal = "https://instadelivery.com.br/fortindelivery";
const nomeLoja = "Açai do Fortin";
...
const corrigirTextoPossivelmenteMalCodificado = (texto = "") => {
  ...
  try {
    ...
    if (corrigido.includes("ï¿½")) return texto;
    return corrigido;
  } catch {
    return texto;
  }
...
const aliasesCatalogoAcai = {
  acai: ["acai", "açai", "tigela", "copo", "barca", "pote", "sobremesa", "sorvete"],
  vitamina: ["vitamina", "vitaminas", "batida"],
  tradicional: ["tradicional", "classica", "clássica", "basica", "básica"],
  especial: ["especial", "especiais", "premium", "copao", "copão", "copoes", "copões"],
  combo: ["combo", "kit", "promocao", "promoção", "familia", "família"],
  adicional: [
    "adicional",
    "extra",
    "topping",
  ...
  fruta: ["fruta", "morango", "banana", "kiwi", "manga", "abacaxi", "uva"],
  bebida: ["agua", "água", "suco", "refrigerante"],
};
...
const buscarCatalogoPorAlias = (textoNormalizado = "") => {
  ...
  const aliasEncontrado = Object.entries(aliasesCatalogoAcai).find(([, termos]) =>
...
const respostaCatalogoSeAplicavel = (textoOriginal = "") => {
  ...
  return { texto: mensagemDirecionarLink, etapa: "menu" };
};
...
const classificarPerfilCompraAcai = (textoNormalizado = "") => {
  const texto = String(textoNormalizado);
  if (
    texto.includes("sem acucar") ||
    texto.includes("semacucar") ||
    texto.includes("zero") ||
    texto.includes("fit") ||
    texto.includes("diet")
  ) {
  ...
  if (
    texto.includes("turbinado") ||
    texto.includes("completo") ||
    texto.includes("premium") ||
    texto.includes("especial") ||
    texto.includes("especiais") ||
    texto.includes("nutella") ||
    texto.includes("leite ninho") ||
    texto.includes("ovomaltine") ||
    texto.includes("paçoca") ||
    texto.includes("pacoquinha")
  ) {
    return "turbinado";
  }
  if (
    texto.includes("frutas") ||
    texto.includes("vitamina") ||
    texto.includes("vitaminas") ||
    texto.includes("morango") ||
    texto.includes("banana") ||
    texto.includes("kiwi") ||
    texto.includes("manga") ||
    texto.includes("abacaxi")
  ) {
    return "frutas";
  }
  if (texto.includes("kids") || texto.includes("infantil") || texto.includes("crianca")) {
    return "kids";
  }
  if (texto.includes("misto") || texto.includes("mix")) {
    return "misto";
  }
  if (texto.includes("tradicional") || texto.includes("simples")) {
    return "tradicional";
  }
  return "tradicional";
};

const montarEstimativaCatalogoAcai = (sugestao) => {
  const linhas = [];
  const avisos = [];
  let total = 0;
  ...
  if (sugestao.acaiCopos > 0) {
    const item = selecionarItemCatalogo("acai");
    adicionarItem(item, sugestao.acaiCopos, "acai");
  }

  if (sugestao.combosFamilia > 0) {
    const item = selecionarItemCatalogo("combo");
    adicionarItem(item, sugestao.combosFamilia, "combo");
  }

  if (sugestao.frutasExtras > 0) {
    const item = selecionarItemCatalogo("fruta");
    if (!item) {
      avisos.push("Sem item de frutas extras no catalogo.");
    } else {
      adicionarItem(item, sugestao.frutasExtras, "frutas extras");
    }
  }

  if (sugestao.adicionaisPorcoes > 0) {
    const item = selecionarItemCatalogo("adicional");
    if (!item) {
      avisos.push("Sem item de adicionais no catalogo.");
    } else {
      adicionarItem(item, sugestao.adicionaisPorcoes, "adicional");
    }
  }
...
const calcularSugestaoCompraAcai = ({ pessoas, duracao, perfil }) => {
  const perfilFinal = perfil || "tradicional";
  const horas = Math.max(1, Number(duracao) || 1);
  const totalPessoas = Math.max(1, Number(pessoas) || 1);
  const multiplicadorTempo = 1 + Math.max(0, horas - 1) * 0.12;

  let acaiCopos = Math.max(1, Math.ceil(totalPessoas * multiplicadorTempo));
  if (perfilFinal === "kids") {
    acaiCopos = Math.max(1, Math.ceil(acaiCopos * 0.8));
  }
  if (perfilFinal === "fit") {
    acaiCopos = Math.max(1, Math.ceil(acaiCopos * 0.9));
  }
  if (perfilFinal === "turbinado") {
    acaiCopos = Math.max(acaiCopos, Math.ceil(totalPessoas * 1.1));
  }

  const combosFamilia =
    perfilFinal === "misto" || totalPessoas >= 6 ? Math.max(1, Math.ceil(totalPessoas / 5)) : 0;
  const adicionaisPorcoes =
    perfilFinal === "turbinado"
      ? Math.max(2, Math.ceil(totalPessoas / 2))
      : Math.max(1, Math.ceil(totalPessoas / 4));
  const frutasExtras =
    perfilFinal === "frutas" || perfilFinal === "misto"
      ? Math.max(1, Math.ceil(totalPessoas / 3))
      : Math.max(1, Math.ceil(totalPessoas / 5));
...
const montarResumoConsultoriaAcai = ({ pessoas, duracao, perfil }) => {
  const sugestao = calcularSugestaoCompraAcai({ pessoas, duracao, perfil });
  const perfilLabel = {
  ...
  const linhas = [
    `Perfeito! Para ${pessoas} pessoas e ${duracao} horas (${perfilLabel}),`,
    "minha sugestao inicial para a Açai do Fortin e:",
  ];

  if (sugestao.acaiCopos > 0) {
    linhas.push(`- Acai: ~${sugestao.acaiCopos} copos/tigelas`);
  }
  if (sugestao.combosFamilia > 0) {
    linhas.push(`- Combos familia: ~${sugestao.combosFamilia} unidade(s)`);
  }
  if (sugestao.frutasExtras > 0) {
    linhas.push(`- Frutas extras: ~${sugestao.frutasExtras} porcoes`);
  }
  if (sugestao.adicionaisPorcoes > 0) {
    linhas.push(
      `- Adicionais: ~${sugestao.adicionaisPorcoes} porcoes de granola, leite ninho, pacoca ou caldas`
    );
  }
  linhas.push("");
  const estimativa = montarEstimativaCatalogoAcai(sugestao);
  if (estimativa.linhas.length) {
    linhas.push("Estimativa com base no catalogo:");
    linhas.push(...estimativa.linhas);
    if (estimativa.total) {
  ...
  linhas.push("Se quiser, me diga tamanhos, sabores e adicionais preferidos ou um orcamento e eu ajusto.");
  linhas.push(`Cardapio rapido: ${linkPrincipal}`);

  return linhas.join("\n");
};

const catalogoPadraoAcai = [
  {
    id: "acai_300",
    nome: "Acaí 300ml",
    categoria: "acai",
  ...
  {
    id: "combo_familia_1l",
    nome: "Combo familia 1L com 4 adicionais",
    categoria: "combo",
    unidade: "combo",
    preco: 49.9,
    estoque: 0,
...
if (!carregarCatalogoDoArquivo()) {
  atualizarCatalogo(catalogoPadraoAcai);
  salvarCatalogo(catalogoPadraoAcai);
}
...
Oi! Sou o assistente comercial do ${nomeLoja}.
Posso te ajudar a escolher o melhor acai e montar seu pedido.
...
const mensagemCompraDireta = `Trabalhamos com acai, combos e complementos para seu pedido sair no ponto certo.
...
const mensagemAtendente = `Certo! Vou pausar o robo por 10 minutos para voce conversar com o atendente.

Depois desse periodo eu volto a responder por aqui.`;
const mensagemAudioNaoEntendido = 'Recebi seu audio, mas nao consegui entender. Pode enviar novamente ou escrever por texto.';
const mensagemAgradecimento = `Que bom falar com voce! Muito obrigado pelo carinho.
...
Se quiser, posso te ajudar com seu pedido de acai, taxa de entrega, horario ou endereco.
...
Cardapio:
${linkPrincipal}`;
const mensagemDirecionarLink = `Perfeito! Para ver o cardapio, valores e montar seu pedido, acesse:
${linkPrincipal}

Se quiser, eu continuo te ajudando por aqui com taxa de entrega, horario ou endereco.`;
const mensagemConsultoriaInicio = `Para ver tamanhos, sabores, adicionais e valores, acesse:
${linkPrincipal}

Se quiser, eu também posso te ajudar com taxa de entrega, horario ou endereco.`;
const mensagemPerguntaPessoas = 'Quantas pessoas vao participar?';
const mensagemPerguntaDuracao = 'Por quantas horas o consumo vai durar?';
const mensagemPerguntaPerfil = 'Qual o perfil? (tradicional / especial / vitaminas / fit)';
const mensagemOrcamento = `Para ver os valores e montar o pedido do jeito certo, acesse:
${linkPrincipal}`;
// =====================================
// DELAY
// =====================================
...
const enviarMensagemComRetry = async (destino, conteudo, opcoes = {}, tentativas = 2) => {
  let ultimoErro = null;

  for (let tentativa = 1; tentativa <= tentativas; tentativa += 1) {
    try {
      return await client.sendMessage(destino, conteudo, opcoes);
    } catch (erro) {
      ultimoErro = erro;
      console.log(
        `Aviso ao enviar mensagem (tentativa ${tentativa}/${tentativas}):`,
        erro?.message || erro
      );
      if (tentativa < tentativas) {
        await delay(600 * tentativa);
      }
    }
  }
...
const buildSystemPrompt = () => {
  const base = [
    'Voce e o assistente comercial do Açai do Fortin, especializado em vendas de açaí.',
    'Responda em portugues, de forma objetiva, simpatica e persuasiva.',
    'Ajude a aumentar o ticket medio sugerindo tamanhos, combos, frutas e adicionais quando fizer sentido.',
    ...
    'Use o catalogo do contexto para responder sobre produtos, tamanhos, sabores e precos.',
    'Quando perguntarem se tem um item, responda se existe no catalogo e se esta disponivel, sem informar quantidade em estoque.',
    'Se o estoque for 0, diga que esta indisponivel no momento.',
    'Use o horario de funcionamento configurado quando perguntarem.',
    'Se perguntarem taxa por bairro, use o mapa taxasEntrega; se a taxa for 0, responda que a entrega e gratis.',
    'Se perguntarem taxa por bairro e nao encontrar, diga que nao atendemos.',
    'Se nao souber algo, peca mais detalhes ao cliente.',
    'Quando o assunto for produtos, precos, sabores, pedidos ou orcamento, direcione sempre para o link do cardapio.',
  ].join(' ');
...
client.on("message", async (msg) => {
  ...
  try {
    ...
    console.log(
      `Evento message: from=${msg?.from || "sem_from"} type=${msg?.type || "sem_type"} fromMe=${Boolean(msg?.fromMe)} hasBody=${Boolean(msg?.body)} hasMedia=${Boolean(msg?.hasMedia)}`
    );
    ...
    if (!msg.from) {
      console.log("Mensagem ignorada: sem remetente.");
      return;
    }

    // BLOQUEIA STORIES
    if (msg.from === "status@broadcast") {
      console.log("Mensagem ignorada: story/status broadcast.");
      return;
    }
    ...
    if (msg.fromMe) {
      console.log("Mensagem ignorada: enviada pelo proprio bot.");
      return;
    }
    ...
    if (!msg.body && !mensagemEhAudio) {
      console.log("Mensagem ignorada: vazia e sem audio.");
      return;
    }
    ...
    if (agora - ultimo < 3000) {
      console.log(`Mensagem ignorada: anti-spam (${agora - ultimo}ms).`);
      return;
    }
    ...
    if (chat.isGroup) {
      console.log("Mensagem ignorada: chat.isGroup retornou true.");
      return;
    }
    ...
    const typing = async () => {
      try {
        ...
        console.log("Aviso: nao foi possivel ativar o indicador de digitacao:", erro?.message || erro);
      }
    };

    const responderComTexto = async (textoResposta) => {
      await typing();
      await enviarMensagemComRetry(msg.from, textoResposta);
      ...
      try {
        ...
        try {
          ...
          console.log("Aviso: nao foi possivel ativar o estado de gravacao:", erro?.message || erro);
        }

        await enviarMensagemComRetry(msg.from, audio, { sendAudioAsVoice: true });
      } finally {
    ...
    if (mensagemEhAudio) {
      ...
      if (!textoOriginal) {
        await typing();
        await enviarMensagemComRetry(msg.from, mensagemAudioNaoEntendido);
        return;
      }
    }

    console.log(
      `Mensagem recebida de ${msg.from}: ${textoOriginal.slice(0, 120).replace(/\s+/g, " ")}`
    );

    const texto = normalizarTexto(textoOriginal);
    const session = registrarSessao(msg.from);
    ...
    if (!aiSempreAtivo && session.etapa === "pedido_escolha") {
      if (gatilhosMenu.test(texto)) {
        await typing();
        await enviarMensagemComRetry(msg.from, montarMenuPrincipal());
        session.etapa = "menu";
      ...
      if (!itemSelecionado) {
        ...
        await enviarMensagemComRetry(
          msg.from,
          "Não consegui identificar a opção. Me diga o número ou o nome."
        );
        return;
      }
    ...
    if (!aiSempreAtivo && session.etapa === "pedido_quantidade") {
      ...
      if (!quantidade || quantidade <= 0) {
        await typing();
        await enviarMensagemComRetry(msg.from, "Me diga a quantidade desejada, por favor.");
        return;
      }
    ...
    if (!aiSempreAtivo && gatilhosCardapio.some((item) => texto.includes(item))) {
      ...
      await typing();
      await enviarMensagemComRetry(msg.from, mensagemCardapio);
      session.etapa = "menu";
      return;
    }
    ...
    if (!aiSempreAtivo && gatilhosCompra.some((item) => texto.includes(item))) {
      ...
      await typing();
      await enviarMensagemComRetry(msg.from, mensagemCompraDireta);
      session.etapa = "menu";
      return;
    }
    ...
    if (!aiSempreAtivo && gatilhosOrcamento.some((item) => texto.includes(item))) {
      await typing();
      await enviarMensagemComRetry(msg.from, mensagemOrcamento);
      session.etapa = "menu";
      return;
    }
    ...
    if (!aiSempreAtivo && gatilhosEndereco.some((item) => texto.includes(item))) {
      ...
      await enviarMensagemComRetry(msg.from, mensagemEndereco);
      session.etapa = "menu";
      return;
    }
    ...
    if (!aiSempreAtivo && gatilhosAgradecimento.some((item) => texto.includes(item))) {
      ...
      await typing();
      await enviarMensagemComRetry(msg.from, mensagemAgradecimento);
      session.etapa = "menu";
      return;
    }
    ...
    if (!aiSempreAtivo && gatilhosCordialidade.some((item) => texto.includes(item))) {
      ...
      await typing();
      await enviarMensagemComRetry(msg.from, mensagemCordialidade);
      session.etapa = "menu";
      return;
    }
    ...
    if (!aiSempreAtivo && gatilhosConfirmacao.some((item) => texto === item || texto.includes(`${item} `) || texto.endsWith(item))) {
      ...
      await typing();
      await enviarMensagemComRetry(msg.from, mensagemConfirmacao);
      session.etapa = "menu";
      return;
    }
    ...
    if (!aiSempreAtivo && gatilhosPosterior.some((item) => texto.includes(item))) {
      ...
      await typing();
      await enviarMensagemComRetry(msg.from, mensagemPosterior);
      session.etapa = "menu";
      return;
    }
    ...
    if (!aiSempreAtivo && gatilhosDespedida.some((item) => texto.includes(item))) {
      ...
      await typing();
      await enviarMensagemComRetry(msg.from, mensagemDespedida);
      session.etapa = "menu";
      return;
    }
...
Digite seu bairro para consultar a taxa e seguir para o pedido.`
    ...
    if (texto === "3") {
        ...
        await enviarMensagemComRetry(msg.from, mensagemHorario);
        return;
      }
      ...
      if (texto === "5") {
        await typing();
        await enviarMensagemComRetry(msg.from, mensagemAtendente);
    ...
    if (session.etapa === "consultoria_pessoas") {
      ...
      if (!pessoas || pessoas <= 0) {
        await typing();
        await enviarMensagemComRetry(msg.from, mensagemPerguntaPessoas);
        return;
      }
      ...
      if (duracao && duracao > 0) {
        ...
        const perfil = classificarPerfilCompraAcai(texto);
        if (perfil) {
          session.consultoria.perfil = perfil;
          await typing();
          await client.sendMessage(
            msg.from,
            montarResumoConsultoriaAcai(session.consultoria)
          );
          session.etapa = "menu";
          return;
        }
        ...
        await typing();
        await enviarMensagemComRetry(msg.from, mensagemPerguntaPerfil);
        session.etapa = "consultoria_perfil";
        return;
      }

      await typing();
      await enviarMensagemComRetry(msg.from, mensagemPerguntaDuracao);
...
Digite outro bairro ou *menu*.`
    ...
    await enviarMensagemComRetry(
      msg.from,
      `Nao consegui entender totalmente.
Me diga o sabor, tamanho ou adicional que voce quer e eu ja te ajudo.
